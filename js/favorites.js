...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!user.favoriteSentences || user.favoriteSentences.length === 0) {
        favSentencesList.innerHTML = `<p style="text-align: center; color: var(--text-color); opacity: 0.7;">لا توجد جمل مفضلة. قم بتكوين جملة واضغط على "حفظ الجملة" ❤️</p>`;
    } else {
    ...
    if (!user.favorites || user.favorites.length === 0) {
        favWordsList.innerHTML = `<p style="text-align: center; color: var(--text-color); opacity: 0.7;">لم تقم بإضافة كلمات للمفضلة 🤍</p>`;
    } else {
...
// ==========================================
// الدوال المساعدة (النطق، المسح، والمايك)
// ==========================================
...
window.startMicTest = function(targetText, btnId) {
    ...
    // سكرول لفوق عشان الطفل يشوف الشاشة الحية
    ...
    recognition.onresult = (event) => {
        ...
        if (finalTranscript !== '') {
            ...
            if (accuracy >= 70 || spokenClean.includes(targetClean)) {
                ...
                liveBox.innerHTML = `✅ ${finalTranscript} <br><span style="color:#4CAF50; font-size:1rem; display:block; margin-top:5px;">(دقة النطق: ${accuracy}% - ممتاز! +10 نقط)</span>`;
