...
const favSentenceBtn = document.getElementById('favSentenceBtn'); // الزرار الجديد
...
window.renderSentence = function() {
    ...
    if (playSentenceBtn) playSentenceBtn.innerHTML = currentLang === 'ar' ? '🔊 انطق الجملة' : '🔊 Play Sentence';
    if (micSentenceBtn) micSentenceBtn.innerHTML = currentLang === 'ar' ? '🎤 جرب بصوتك' : '🎤 Use Mic';
    if (favSentenceBtn) favSentenceBtn.innerHTML = currentLang === 'ar' ? '❤️ حفظ الجملة' : '❤️ Save Sentence';
    if (clearSentenceBtn) clearSentenceBtn.innerHTML = currentLang === 'ar' ? '🗑️ مسح' : '🗑️ Clear';

    if (currentSentence.length === 0) {
        const placeholder = document.createElement('p');
        placeholder.className = 'placeholder-text';
        placeholder.id = 'sentencePlaceholder';
        placeholder.textContent = currentLang === 'ar' ? 'اضغط على الكروت لتكوين جملة...' : 'Click cards to build a sentence...';
    ...
    currentSentence.forEach((wordObj, index) => {
        ...
        wordEl.style.cursor = 'pointer'; // خليناها زرار
...
if (favSentenceBtn) {
    favSentenceBtn.addEventListener('click', () => {
        ...
        if (isExists) {
            alert('الجملة دي محفوظة عندك بالفعل! ❤️');
        } else {
            user.favoriteSentences.push([...currentSentence]); // بناخد نسخة منها
            localStorage.setItem('currentUser', JSON.stringify(user));
            localStorage.setItem(user.email, JSON.stringify(user));
            alert('تم حفظ الجملة في المفضلة بنجاح! ✅');
        }
    });
}

// النطق (فقط نطق، بدون إعطاء نقاط)
if (playSentenceBtn) {
        ...
        recognition.onresult = (event) => {
            ...
            if (finalTranscript !== '') {
                ...
                if (accuracy >= 70 || spokenClean.includes(targetClean)) {
                    ...
                    liveCaptionBox.innerHTML = `✅ ${finalTranscript} <br><span style="color:#4CAF50; font-size:1rem; display:block; margin-top:5px;">(دقة النطق: ${accuracy}% - ممتاز! +10 نقط)</span>`;
                    ...
                    liveCaptionBox.innerHTML = `❌ ${finalTranscript} <br><span style="color:#e74c3c; font-size:1rem; display:block; margin-top:5px;">(دقة النطق: ${accuracy}% - المطلوب: ${targetText}) حاول تاني!</span>`;
                }
            }
        };
